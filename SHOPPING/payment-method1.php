...
$userdata=mysqli_query($con,"select u.name name, u.contactno phone,u.lcode lcode from users u  where u.id='".$_SESSION['id']."'");
...
//ini_set("sendmail_from", "contact@rkjr.co.in");
    ...
    */
	...
	/*$from_add = "shrey@rkjrrampur.in"; 

	$to_add = "shreybassi@gmail.com"; //<-- put your yahoo/gmail email address here
...
*/
$from = "rkjrshop@gmail.com";
$toemail = "shreybassi@gmail.com";
...
$userdata=mysqli_query($con,"select u.name name, u.contactno phone,u.lcode,u.email email from users u,orders o where u.id= o.userId order by o.order_id desc limit 1");
$userrow = mysqli_fetch_array($userdata);
$orddata=mysqli_query($con,"select max(order_id) oo from orders");
$ordrow = mysqli_fetch_array($orddata);
$to1=$userrow['email'];
		//$Sender = "info@rkjr.co.in";
		//$Recipient = "shreybassi@gmail.com,saurabhbassi@gmail.com,sunilbassi25@hotmail.com";		
		//$custName = $_SESSION['username'];
		$subject = "RKJR-WHOLESALE: Thank You for placing an order ".$userrow['name'];
		
		//$custPhone = $userrow['phone'];
		//$custEmail = $userrow['lcode'];
		$message = "
		Hi ".$userrow['name'].",
			Please confirm your details.
				
					Customer Name  : ".$userrow['name']."
					Customer Phone : ".$userrow['phone']."
					Order ID       : ".$ordrow['oo']."
					Customer Email : ".$userrow['email']."
			 ** This is an auto generated mail. In case of any discrepancy please Reply All to this email.
			 ___________________ 
			 Regards, 
			 RKJR Shopping Admin Team. "; 
		//$Type = "HTML";
		//$mail->send();
if ( strpos($hostname, 'cpnl') === FALSE ) //if not cPanel
		...
		$mail->From = $from;
		$mail->FromName = 'RKJR Admin';
		
		$mail->AddAddress($to1, 'Customer');
		$mail->AddBCC('shreybassi@gmail.com', 'Shrey Bassi');
$mail->AddBCC('sunilbassi@gmail.com', 'Sunil Bassi');
$mail->AddBCC('saurabhbassi@gmail.com', 'Saurabh Bassi');
		...
		mysqli_query($con,"update orders set 	paymentMethod='".$_POST['paymethod']."' where userId=(select max(id) from users) and paymentMethod is null ");
		unset($_SESSION['cart']);
		if($_POST['paymethod']=="COD"){
		header('location:latest-order-guest.php');
		}
	else{
